...
- `bets/{user_id}_{match_id}` — **the doc id IS the client bet key** (`` `${user_id}_${match_id}` ``). Fields: `user_id, match_id, home, away`. `setDoc` (no merge) is the upsert.
...
**Scoring / bet outcome.** `betOutcome(bet, resultHome, resultAway)` in `lib/data.ts` classifies a bet as `perfect` (exact score), `win` (correct winner/draw via `Math.sign`), `loss`, or `none`. `MatchCard` uses it on finished matches to color the player's pick strip (gold = perfect, green = won, red = lost) and to show the prediction on upcoming cards too. `Leaderboard.tsx` independently computes points over finished matches (perfect = 3 pts + counts as an "exact", win = 1 pt), sorted by points then exacts — keep the two in sync if scoring changes.
